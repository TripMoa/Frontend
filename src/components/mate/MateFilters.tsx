...
export function MateFilters({
  ...
  return (
    <div className={`bg-white p-6 mb-8 ${styles.filters}`}>
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <DatePicker 
            selected={dateFilter} 
            onChange={(date: Date | null) => { setDateFilter(date); setCurrentPage(1); }} 
        ...
        <div className="relative flex-1 min-w-[140px] dropdown-gender">
          <button
            ...
            className={`w-full px-3 py-2.5 border-2 border-black text-left text-sm font-mono font-bold flex items-center justify-between transition-all ${genderFilter !== "전체" ? styles.bgActive : "bg-white text-black hover:bg-[#f5f5f5]"}`}
          >
          ...
          {showGenderDropdown && (
            <div className={`absolute top-full left-0 right-0 mt-1 bg-white z-50 overflow-hidden ${styles.dropdown}`}>
              {GENDER_OPTIONS.map((option, idx) => (
                <button
                  key={option}
                  type="button"
                  onClick={() => { setGenderFilter(option); setShowGenderDropdown(false); setCurrentPage(1); }}
                  className={`w-full px-4 py-2.5 text-left text-sm font-mono font-bold transition-colors ${idx !== GENDER_OPTIONS.length - 1 ? "border-b border-black/10" : ""} ${genderFilter === option ? styles.bgActive : "bg-white text-black hover:bg-[#eee]"}`}
                >
                  {option}
                </button>
              ))}
        ...
        <div className="relative flex-1 min-w-[140px] dropdown-age">
          <button
            ...
            className={`w-full px-3 py-2.5 border-2 border-black text-left text-sm font-mono font-bold flex items-center justify-between transition-all ${ageFilter !== "전체" ? styles.bgActive : "bg-white text-black hover:bg-[#f5f5f5]"}`}
          >
            ...
            <div className={`absolute top-full left-0 right-0 mt-1 bg-white z-50 overflow-hidden ${styles.dropdown}`}>
              {AGE_OPTIONS.map((option, idx) => (
                <button
                  key={option}
                  type="button"
                  onClick={() => { setAgeFilter(option); setShowAgeDropdown(false); setCurrentPage(1); }}
                  className={`w-full px-4 py-2.5 text-left text-sm font-mono font-bold transition-colors ${idx !== AGE_OPTIONS.length - 1 ? "border-b border-black/10" : ""} ${ageFilter === option ? styles.bgActive : "bg-white text-black hover:bg-[#eee]"}`}
                >
                  {option}
                </button>
              ))}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {ALL_TAGS.map((tag) => (
          <button 
            ...
            className={`px-3 py-1.5 border-2 border-black text-xs transition-all font-bold ${selectedTags.includes(tag) ? `${styles.bgActive} ${styles.tagActive}` : "bg-white text-black hover:bg-[#f5f5f5]"}`}
          >
            {tag}
          </button>
        ))}
